"""Mongo message repository adapter."""
...
from omniagent.persistence.backends.mongo.repositories.serializers import message_document_to_record


@dataclass(slots=True)
class MongoMessageRepository:
    message_model: type[Document]
    ...
    async def list_by_session(self, session_id: str, page: int, page_size: int) -> Page[MessageRecord]:
        ...
        return Page(
            items=[message_document_to_record(message) for message in messages],
    ...
    async def list_all_by_session(self, session_id: str) -> list[MessageRecord]:
        ...
        return [message_document_to_record(message) for message in messages]
