"""Mongo session repository adapter."""
...
from omniagent.persistence.backends.mongo.repositories.serializers import session_document_to_record
...
class MongoSessionRepository:
    ...
    async def get_by_id_and_client_id(self, session_id: str, client_id: str) -> SessionRecord | None:
        ...
        if session is None:
            return None
        return session_document_to_record(session)
    ...
    async def list_by_client_id(self, client_id: str, page: int, page_size: int) -> Page[SessionRecord]:
        ...
        return Page(
            items=[session_document_to_record(session) for session in sessions],
